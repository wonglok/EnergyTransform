<template>
  <div class="full u-layer-base">
    ...
    <Scene @ready="(v) => { scene = v; init() }">
      ...
      <!-- OMG -->
      <slot name="scene" :self="self" :camera="camera" :scene="scene" :renderer="renderer" :execStack="execStack"></slot>
    </Scene>
...
import FreeJS from '../../vfx/FreeJS'
import * as THREE from 'three'
import * as AudioService from '../../vfx/InterfaceService/mic-timed.js'
...
export default {
  ...
  data () {
    return {
      ...
      world: false,
      Settings: {
        camPosition: { x: 0.0, y: 0.0, z: 350.0 },
        bloomPass: {
  ...
  methods: {
    ...
    render () {
      let { scene, camera, renderer, composer, mouse, rect, useBloom } = this

      this.execStack.forEach(e => e({ mouse, rect }))

      if (scene && camera && renderer && composer && useBloom) {
        composer.render()
